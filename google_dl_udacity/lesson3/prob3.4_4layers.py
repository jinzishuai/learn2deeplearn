...
with graph.as_default():
  ...
  valid_prediction = tf.nn.softmax( tf.matmul( \
      tf.nn.relu(tf.matmul(tf.nn.relu(tf.matmul(tf_valid_dataset, weights1) + biases1), weights2) + biases2) \
                                   , weights3) + biases3 )
  
  test_prediction = tf.nn.softmax(  tf.matmul( \
      tf.nn.relu(tf.matmul(tf.nn.relu(tf.matmul(tf_test_dataset, weights1) + biases1), weights2) + biases2) \
                                   , weights3) + biases3 )
